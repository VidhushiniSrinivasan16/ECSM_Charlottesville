...
```{r}
setwd("/Users/v.srinivasan/Downloads/ACM_Emoji_Vidhushini/")
...
library(dplyr)
require(parallel) 
raw_tweets <- parallel_match(new_data_solidarity$text, new_matchto, new_description, mc.cores=4)%>% 
...
##Word cloud
```{r}
I_full <-  new_data_solidarity%>%dplyr::filter(str_detect(text,regex('I\\s+ |Me\\s+|Mine\\s+|My\\s+|i\\s+|me\\s+|mine\\s+|my\\s+',ignore_case = T)))
We_full <-  new_data_solidarity%>%dplyr::filter(str_detect(text,regex('We\\s+|Our\\s+|Ours\\s+|Us\\s+|we\\s+|our\\s+|ours\\s+|us\\s+',ignore_case = T)))
Neither_full <-  new_data_solidarity%>%dplyr::filter(!str_detect(text,regex('I\\s+|Me\\s+|Mine\\s+|My\\s+|i\\s+|me\\s+|mine\\s+|my\\s+',ignore_case = T)))%>%dplyr::filter(!str_detect(text,regex('We\\s+|Our\\s+|Ours\\s+|Us\\s+|we\\s+|our\\s+|ours\\s+|us\\s+',ignore_case = T)))
```

```{r}
I_full <-  new_data_solidarity%>%dplyr::filter(str_detect(text,"i "))
We_full <-  new_data_solidarity%>%dplyr::filter(str_detect(text,"we "))
Neither_full <-  new_data_solidarity%>%dplyr::filter(!str_detect(text,"i "))%>%dplyr::filter(!str_detect(text,"we "))
```
...
```{r}
corpus_en <- data_solidarity$body
...
```{r}
boot_combined=read.csv2(file="VP_mono_eng.csv", header=TRUE, sep=",", stringsAsFactors = FALSE, fileEncoding = "UTF-8")
```
...
```{r}
write.csv(boot_combined,"anova_analysis_charlottesville_tweetlength.csv")
```
...
barplot(counts, main="We Tweets Distribution", 
   xlab="Month")
```
